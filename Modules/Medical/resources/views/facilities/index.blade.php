...
@section('page-script')
    ...
    <script>
        window.BulkURL = "{{ route('facilities.bulk_action') }}";
...
@section('content')
    @can('statics_facilities')
        @include('medical::facilities.partials.facilities_statics')
    @endcan

    <div class="row">
        <!-- Filter Card (Initially Hidden) -->
        <div id="filter-card" class="col-3 hidden">
            @include('medical::facilities.partials.facilities_search_filter')
        </div>
        <!-- DataTable Card -->
        <div id="datatable-card" class="col-12 card">
            @include('medical::facilities.partials.facilities_nav')
                ...
                <table class="table gms-table">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('medical::messages.facilities.table.name_en') }}</th>
                            <th>{{ __('medical::messages.facilities.table.name_ar') }}</th>
    ...
    @include('medical::facilities.partials.facility_create')
...
@push('script-page')
    @php
        $table_data = [
            'url' => route('facilities.data', 'null'),
        ];
        $bulk_actions = [];
        $filters = [
            'd.name' => '.searchName',
        ...
        $button = [
            'name' => __('medical::messages.facilities.button.create_facility'),
    ...
    @include('layouts.sections.datatable', [
        ...
        'restore_permission' => 'restore_facilities',
